...
public class Nether implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        String netherPerm = plugin.getConfig().getString("permissions.comando-nether");
        if(!(sender instanceof Player)){
            sendLog("Por qué intentarías usar ese comando desde la consola?",null,3);
        }else{
            if(sender.hasPermission(netherPerm)){
                rama.ubcminas.CommandExecutor CE = new rama.ubcminas.CommandExecutor(plugin);
                CE.executeCommand((Player) sender,"nether", 0);
                if(plugin.debugMode){
                    sendLog("Enviando orden para ejecutar comando de nether a "+ sender.getName(), null, 2);
                }
            }else{
